...
class AuthorizationGraph:
    ...
    def __init__(self):
        ...
        # NEW: carries forward the (patched) collector's collection_issues
        # array -- ACCESS_DENIED / ENUMERATION_BLOCKED / UNAVAILABLE records
        # for objects the collector could not fully enumerate. Defaults to
        # an empty list so older collector output (without this field)
        # loads with no behaviour change.
        self.collection_issues: List[Dict] = []
    ...
    def load_from_json(self, filepath: str) -> bool:
        ...
        try:
            ...
            # NEW: carry forward collection_issues from the (patched)
            # collector. The collector now reports ACCESS_DENIED /
            # ENUMERATION_BLOCKED / UNAVAILABLE conditions explicitly
            # instead of silently returning 0/empty for objects it couldn't
            # fully enumerate. Without this, that signal was dropped here --
            # the graph (and everything downstream: analyzer, refiner,
            # visualizer) would have no way to know "0 ACL findings" might
            # mean "couldn't see this part of the domain" rather than "this
            # part is clean." Older collector output without this field
            # loads exactly as before (empty list, no behaviour change).
            self.collection_issues = data.get('collection_issues', [])
            self.statistics['collection_issues'] = len(self.collection_issues)
            if self.collection_issues:
                by_status: Dict[str, int] = {}
                for issue in self.collection_issues:
                    status = issue.get('status', 'UNKNOWN')
                    by_status[status] = by_status.get(status, 0) + 1
                print(f"    [!] Collector reported {len(self.collection_issues)} collection issue(s): "
                      f"{', '.join(f'{k}={v}' for k, v in by_status.items())}")
                print(f"    [!] Carried into exported graph JSON ('collection_issues' / "
                      f"'statistics.collection_issues') -- some findings downstream may be "
                      f"INCOMPLETE rather than confirmed-clean.")
    ...
    def get_collection_issues(self) -> List[Dict]:
        """Return collector-reported ACCESS_DENIED / ENUMERATION_BLOCKED / UNAVAILABLE records"""
        return self.collection_issues
    ...
    def export_graph_data(self, output_file: str = "graph_data.json") -> bool:
        ...
        try:
            graph_data = {
                ...
                # NEW: forwarded so refiner.py / visualizer can surface
                # collector-side gaps instead of only ever seeing graph-
                # builder-side completeness.
                'collection_issues': self.collection_issues,
                'nodes': [],
                'edges': []
            }
