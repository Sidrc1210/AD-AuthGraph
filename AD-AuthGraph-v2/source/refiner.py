        ...
        for target_id, chains in self.analysis.get("acl_abuse_chains", {}).items():
            for chain in chains:
                ...
                findings.append({
                    ...
                    # analyzer.c compact mode may emit a bounded sample here;
                    # carry count/truncation metadata forward so the visualizer
                    # does not mistake the sample for an exhaustive list.
                    "authorities_gained": chain.get("authorities_gained", []),
                    "authorities_gained_count": chain.get(
                        "authorities_gained_count",
                        len(chain.get("authorities_gained", [])),
                    ),
                    "authorities_gained_truncated": chain.get(
                        "authorities_gained_truncated",
                        False,
                    ),
                })
